...
int main() {
	...
	ofstream output;
	std::string line;
	file.open("C:\\Users\\alidu\\source\\repos\\HackerrankExamples\\HackerrankExamples\\input06.txt");
	output.open("C:\\Users\\alidu\\source\\repos\\HackerrankExamples\\HackerrankExamples\\output06.txt");
	if (file.is_open())
	{
		getline(file, line);
		std::string first;
		...
		capacity = stoi(line.substr(index + 1));
		LRUCacheMp l(capacity);
		while (getline(file, line))
		{
			...
			if (command == "get") {
				int key;
				key = stoi(line.substr(index + 1));
				output << l.get(key) << endl;
				//cout << l.get(key) << endl;
			}
			else if (command == "set") {
				int key, value;
				...
				index = line.find(' ');
				key = stoi(line.substr(0, index));
				value = stoi(line.substr(index + 1));
				l.set(key, value);
			}
		}
	}
